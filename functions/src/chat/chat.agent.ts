...
function insertTimestampExplanation(
  systemPrompt: string,
  explanation: string,
): string {
  if (!systemPrompt) return explanation;
  const responseFormatIndex = systemPrompt.indexOf('\n--- Response format ---');
  if (responseFormatIndex !== -1) {
    const beforeFormat = systemPrompt.slice(0, responseFormatIndex).trimEnd();
    const formatPart = systemPrompt.slice(responseFormatIndex);
    return `${beforeFormat}\n\n${explanation}${formatPart}`;
  }
  return `${systemPrompt}\n\n${explanation}`;
}
...
export async function getAgentChatMessage(
  ...
  if (useMessageFormat && isPrivateChat) {
    const includeTimestamps =
      ...
      Boolean(promptConfig.chatSettings?.includeTimestampsPrivateChat);
  ...
  // Only strip timestamps when generated by a mediator in private chat with the feature enabled
  const shouldStripTimestamps =
    user.type === UserType.MEDIATOR &&
    stage.kind === StageKind.PRIVATE_CHAT &&
    Boolean(promptConfig.chatSettings?.includeTimestampsPrivateChat);
  const outgoingMessage = shouldStripTimestamps
    ? stripTimestampPrefix(message)
    : message;
...
async function convertToMessageFormat(
  ...
  // Insert timestamp format explanation before JSON response format (if present)
  if (includeTimestampsPrivateChat) {
    systemPrompt = insertTimestampExplanation(
      systemPrompt,
      PRIVATE_CHAT_TIMESTAMP_EXPLANATION,
    );
  }
