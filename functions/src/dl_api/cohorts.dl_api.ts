...
/**
 * Reset a cohort in place: soft-delete its participants, wipe its stage data,
 * and re-initialize the cohort document under the SAME id, so existing join
 * links keep working while the cohort starts fresh.
 */
export async function resetCohort(
  req: DeliberateLabAPIRequest,
  res: Response,
): Promise<void> {
  if (!hasDeliberateLabAPIPermission(req, 'write')) {
  ...
  // Verify ownership (only creator can reset cohorts)
  await verifyExperimentOwnership(experimentId, experimenterId);
  ...
  const existing = cohortDoc.data() as CohortConfig;
  ...
  // Wipe the cohort document and its subcollections (public stage data,
  // chats), then recreate the SAME id with the same metadata and participant
  // config; createCohortInternal re-initializes stage data and cohort-scope
  // variables exactly as a newly created cohort gets them.
  await app.firestore().recursiveDelete(cohortRef);

  const freshConfig = createCohortConfig({
    id: cohortId,
    ...
    participantConfig: existing.participantConfig,
  });

  await app.firestore().runTransaction(async (transaction) => {
    await createCohortInternal(transaction, experimentId, freshConfig);
  });
