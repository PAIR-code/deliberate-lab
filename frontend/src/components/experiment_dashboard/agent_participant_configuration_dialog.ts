...
export class AgentParticipantDialog extends MobxLitElement {
        ...
        <pr-button
          ...
          @click=${() => {
            ...
            if (this.cohort && this.agentId) {
              const agent =
                this.experimentManager.agentPersonaMap[this.agentId] ?? this.experimentEditor.getAgentParticipant(this.agentId)?.persona;
  ...
  private renderAgentPersona() {
    ...
    const renderEmptyMessage = () => {
      if (this.experimentManager.agentParticipantPersonas.length + this.experimentEditor.agentParticipants.length > 0) {
        return nothing;
      }
      return html`
        <div class="error">
          No agent personas have been configured. Please add a new agent persona.
        </div>
      `;
    };
