...
export function getChipNegotiationStageConfigs(): StageConfig[] {
  ...
  stages.push(CHIP_INFO_STAGE_1);
  stages.push(CHIP_INFO_STAGE_2);
  stages.push(CHIP_INFO_STAGE_3);
  stages.push(CHIP_INFO_STAGE_4);
  stages.push(CHIP_INFO_STAGE_5);
  stages.push(CHIP_INFO_STAGE_6);
...
// Info stage for chip negotiation
// ****************************************************************************
const CHIP_INFO_STAGE_1 = createInfoStage({
  id: 'info1',
  game: StageGame.CHP,
  name: 'Overview and setup (1/2)',
  infoLines: [
    'Today, you will be playing a trading game with two other participants. You will start with:',
    '* 🔴 100 **Red** chips',
    '* 🟢 100 **Green** chips',
    '* 🔵 100 **Blue** chips',
  ],
...
const CHIP_INFO_STAGE_2 = createInfoStage({
  id: 'info2',
  game: StageGame.CHP,
  name: 'Overview and setup (2/2)',
  infoLines: [
    '**Valuations:**',
    '* Each participant has a unique value for **Red** and **Blue** chips, chosen randomly between $0.01 and $0.10.',
    '* For example, Participant A might value Red chips at $0.02 each and Blue chips at $0.07 each, while Participant B might value Red chips at $0.08 each and Blue chips at $0.03 each.',
    '* All participants value **Green** chips the same, at $0.05 per chip.',
    '* Each participant only knows their own valuations, not anyone else’s.',
    '\n**What This Means:**',
...
const CHIP_INFO_STAGE_3 = createInfoStage({
  id: 'info3',
  game: StageGame.CHP,
  name: 'Gameplay structure',
  infoLines: [
    '**Number of Rounds:**',
    'The game consists of **3 rounds** of trading. During each round, you will have an opportunity to propose a trade to the other participants, and to evaluate trades offerred by the other participants.',
    '\n**Turn Order:**',
    'Within each round, every participant will get exactly one turn to make an offer in a fixed, randomly assigned order.',
  ],
});

const CHIP_INFO_STAGE_4 = createInfoStage({
  id: 'info4',
  game: StageGame.CHP,
  name: 'Offers',
  infoLines: [
    '**Offers:**',
    '* On their turn, a participant may propose **one offer** to all other active participants.',
    ...
    'For example, a participant might say: “I will give 20 Red chips in exchange for 10 Blue chips.”',
    '',
    '**Constraints on Offers:**',
    '* A participant cannot offer more chips than they currently hold.',
    '* For instance, if you only have 50 Red chips, you cannot offer 60 Red chips.',
    '* You must request only **one color** of chips in return.',
    '* The chips you are offering must be all of the **same color**. You cannot mix colors in what you’re offering. (However, the color you request in return can be different from the one you’re offering, as is the point of the trade.)',
  ],
});

const CHIP_INFO_STAGE_5 = createInfoStage({
  id: 'info5',
  game: StageGame.CHP,
  name: 'Accepting offers and completing trades',
  infoLines: [
    '**Acceptance Process:**',
    '* Once an offer is presented, **every other active participant** (i.e., those not currently making the offer) gets a chance to **accept or decline**.',
    '* Participants make their decisions simultaneously and privately.',
    ...
    '',
    '**After an Offer:**',
    '* If a trade occurs, the involved participants adjust their chip holdings accordingly.',
    '* If no trade occurs (because no one accepted), nothing changes.',
    '* After the offer is concluded (regardless of outcome), the turn passes to the next participant.',
    '',
    '**End of Round and Game Conclusion:**',
    '* After all 3 participants have made an offer, the round ends.',
    '* The game proceeds for a total of 3 rounds (each participant making an offer each round).',
    '* Once all 3 rounds are completed, the game ends.',
    '* Participants then can calculate their final payoff by multiplying the number of chips they hold in each color by their private valuation for that color (except Green, which is known to be $0.05 for everyone).',
  ],
});

const CHIP_INFO_STAGE_6 = createInfoStage({
  id: 'info6',
  game: StageGame.CHP,
  ...
  infoLines: [
    ...
    '\n**1. Random Selection of Game:**',
    '* Out of the two negotiation games you played, we will **randomly select one** for payment.',
    '\n**2. Valuations and Final Holdings:**',
    '* We will use the **valuations and final chip counts** from the chosen game.',
    '  - For each color of chip you hold at the end of that game, we will multiply the number of chips by your assigned valuation for that color.',
    '  - We will then add up these amounts across Red, Green, and Blue chips.',
    '\n**3. Your Final Payment:**',
    '* The sum of the values of all your chips in the selected game will determine your final payment amount.',
    '  This amount is what you will receive for your participation.',
    '\n**Example:**',
    '* Suppose the second game is chosen. At the end of that game, you have:',
    '  - 80 Red chips valued at $0.03 each',
    '  - 120 Green chips valued at $0.05 each',
    '  - 90 Blue chips valued at $0.07 each',
    '  Your payment would be calculated as:',
    '  [(80 x $0.03) + (120 x $0.05) + (90 x $0.07) = $2.40 + $6.00 + $6.30 = $14.70]',
...
const CHIP_INFO_TRANSFER_STAGE = createInfoStage({
  ...
  infoLines: [
    ...
    '\n**1. New Chip Values:**',
    '* You will receive a different valuation for your chips this time, with the **exception of the Green chip**, which remains the same for everyone.',
    '\n**2. New Turn Order:**',
    '* The order in which players take their turns will be **randomly rearranged** for this round.',
