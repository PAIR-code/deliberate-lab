...
export function getConsensusTopicTemplate(
  ...
  // Pass the topics to the stage config generator
  const stageConfigs = getConsensusStageConfigs(topicsCsv);

  return createExperimentTemplate({
    experiment: createExperimentConfig(stageConfigs, { metadata: CONSENSUS_METADATA }),
    stageConfigs,
    agentMediators: [],
    agentParticipants: [],
  });
...
const PRE_DISCUSSION_PERSONAL_SURVEY = createSurveyStage({
  ...
  questions: [
    createScaleSurveyQuestion({
      ...
      lowerValue: 0, upperValue: 10, stepSize: 1,
    }),
    createScaleSurveyQuestion({
      id: '78658ccf-6415-4a49-9d36-25ddd555ec19',
      questionTitle: 'People feel comfortable having hard conversations with me',
      lowerValue: 0, upperValue: 10, stepSize: 1,
    }),
    createScaleSurveyQuestion({
    ...
    createMultipleChoiceSurveyQuestion({
      ...
      options: [
        { id: '79ad69d7-f877-4123-a689-a2118078b0c6', text: 'Socratic', imageId: '' },
        { id: '6e344f69-c935-445a-993b-1bb212d7d406', text: 'Argumentative', imageId: '' },
        { id: 'c780a539-878a-4459-a52e-3272da98e565', text: 'Listening [passive]', imageId: '' },
        { id: '2a7a9e81-18e5-4c26-92ea-5de02e812543', text: 'Other', imageId: '' },
      ],
    }),
  ],
});
...
// ****************************************************************************
// DYNAMIC STAGE FACTORIES (These are called for each topic)
// ****************************************************************************
  ...
  return createSurveyStage({
    ...
    questions: [
      ...
      createScaleSurveyQuestion({
        ...
        lowerValue: 0, upperValue: 10, stepSize: 1,
        lowerText: "Greatly Bad", upperText: "Greatly Good",
      }),
      createScaleSurveyQuestion({
        id: `qualified-${index}`,
...
/** Creates a chat room for a specific topic. */
 ...
 * NEWLY ADDED!
 */
function createPostDiscussionSurveyStage(topic: string, index: number): StageConfig {
    return createSurveyPerParticipantStage({
        ...
        questions: [
            ...
            createScaleSurveyQuestion({
                ...
                questionTitle: "I felt comfortable discussing controversial topics with [other]",
                lowerValue: 0, upperValue: 10, stepSize: 1,
            }),
            createScaleSurveyQuestion({
            ...
            // Generic questions about the other participant
            createMultipleChoiceSurveyQuestion({
                id: `debate-style-${index}`,
                questionTitle: "[other]'s debate style is [TODO: find taxonomy of debate styles]",
                options: [
    ...
    return createSurveyPerParticipantStage({
        ...
        questions: [
            createTextSurveyQuestion({
                ...
                questionTitle: 'Quote an important point made by the participant in the discussion!',
            }),
            createTextSurveyQuestion({
                id: `salient-point-2-${index}`,
                questionTitle: 'Quote another important point made by the participant in the discussion!',
            }),
            createTextSurveyQuestion({
...
function getConsensusStageConfigs(topicsCsv: string): StageConfig[] {
  ...
  const topics = topicsCsv.split(',').map(topic => topic.trim()).filter(topic => topic);
  ...
  topics.forEach((topic, index) => {
    ...
    // Add the block of stages for this topic in the correct order.
    stages.push(
        topicSurveyStage, 
        groupChatStage, 
        postDiscussionSurvey, // <-- Placed after the chat
        salientPointsStage
    );
  });
