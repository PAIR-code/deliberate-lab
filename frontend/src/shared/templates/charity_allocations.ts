import {
  // ⛔️ REMOVED: createMultiAssetAllocationStage,
  createChatStage,
...
export function getCharityDebateTemplate(
  ...
  debateRoundsCharities.forEach((charityGroup, index) => {
    ...
    const preAllocationStage = createAllocationSurveyStage(
      `vote-round-${roundNum}-pre`,
      `Allocation (Pre-Discussion): Round ${roundNum}`,
      charityGroup,
    );
    stages.push(preAllocationStage);
    ...
    const postAllocationStage = createAllocationSurveyStage(
      `vote-round-${roundNum}-post`,
      `Allocation (Post-Discussion): Round ${roundNum}`,
      charityGroup,
    );
    stages.push(postAllocationStage);
...
function createAllocationSurveyStage(
  ...
  const allocationQuestions: ScaleSurveyQuestion[] = charityGroup.map(
    (charityName) =>
      createScaleSurveyQuestion({
        id: `${id}-${charityName.replace(/[^a-zA-Z0-9]/g, '-')}`, // Create a safe ID
        questionTitle: `Allocation for: ${charityName}`,
        useSlider: true, // Use the slider UI
        lowerValue: 0,
        upperValue: 100,
        lowerText: '0%',
        upperText: '100%',
